#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Round 10: add section 1.7 (the delivered netlist does not describe the layout) and
a red flag into B.5."""
import io
import os
...
NEW = '''    p1.append("<h3>1.7 \u6700\u540e\u4e00\u6761 LVS \u9519\u8bef\u7684\u771f\u56e0\uff1a"
    ...
    p1.append(tbl(["\u9879", "\u5b9e\u6d4b"], [
        ...
        ["<code>HFSNET_8</code> \u7684\u9a71\u52a8", b("\u6ca1\u6709\uff08\u5168\u6587\u4ef6\u65e0 assign / wire / \u4efb\u4f55 .Y\uff09", 2)],
    ...
    p1.append(tbl(["\u5c1d\u8bd5", "\u6539\u6cd5", "v2lvs", "LVS \u7ed3\u679c"], [
        ["\u7b2c 1 \u7248", "\u81ea\u52a8\u6362\u884c\u4e22\u4e86\u7aef\u53e3\u8868\u7684\u9017\u53f7", b("rc=1\uff0c\u65e0 CDL", 2),
...
A = "    # ---- P2 \u65f6\u5e8f ----"
i = t.find(A)
print("P2 anchor at %d" % i)
assert i > 0
t = t[:i] + NEW + t[i:]

# --- B.5: add the netlist self-consistency red flag ---
B5 = '    pb.append("<h3>B.6 '
j = t.find(B5)
print("B.6 anchor at %d" % j)
assert j > 0
FLAG = '''    pb.append(note("<b>\u7b2c 10 \u8f6e\u65b0\u589e\u7684\u6700\u9ad8\u4f18\u5148\u7ea7\u7ea6\u675f</b>\uff1a"
                   "\u4ea4\u4ed8\u7684 P&R \u7f51\u8868\u81ea\u8eab\u4e0d\u81ea\u6d3d \u2014\u2014 "
